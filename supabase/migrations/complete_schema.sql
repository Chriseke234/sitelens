-- SiteLens Complete Pre-Launch Schema & Storage Setup
-- Copy and run this script in your Supabase SQL Editor: https://supabase.com/dashboard/project/cnxclvpbyegcfoaevtoe/sql/new
...
DO $$ BEGIN
  CREATE POLICY "Users can view pages of own audits" ON public.audit_pages FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.audits WHERE audits.id = audit_pages.audit_id AND audits.user_id = auth.uid())
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Users can insert pages for own audits" ON public.audit_pages FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM public.audits WHERE audits.id = audit_pages.audit_id AND audits.user_id = auth.uid())
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE POLICY "Users can view issues of own audits" ON public.audit_issues FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.audits WHERE audits.id = audit_issues.audit_id AND audits.user_id = auth.uid())
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Users can insert issues for own audits" ON public.audit_issues FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM public.audits WHERE audits.id = audit_issues.audit_id AND audits.user_id = auth.uid())
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE POLICY "Users can view own media scans" ON public.media_scans FOR SELECT USING (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Users can insert own media scans" ON public.media_scans FOR INSERT WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Users can update own media scans" ON public.media_scans FOR UPDATE USING (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- POLICIES FOR MEDIA EVIDENCE
DO $$ BEGIN
  CREATE POLICY "Users can view evidence of own media scans" ON public.media_evidence FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.media_scans WHERE media_scans.id = media_evidence.media_scan_id AND media_scans.user_id = auth.uid())
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Users can insert evidence for own media scans" ON public.media_evidence FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM public.media_scans WHERE media_scans.id = media_evidence.media_scan_id AND media_scans.user_id = auth.uid())
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- POLICIES FOR AUDIT AI REPORTS
DO $$ BEGIN
  CREATE POLICY "Users can view AI reports of own audits" ON public.audit_ai_reports FOR SELECT USING (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Users can insert AI reports for own audits" ON public.audit_ai_reports FOR INSERT WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Users can update AI reports for own audits" ON public.audit_ai_reports FOR UPDATE USING (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY "Users can delete AI reports for own audits" ON public.audit_ai_reports FOR DELETE USING (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE POLICY "Authenticated Users Upload Screenshots" ON storage.objects FOR INSERT WITH CHECK (
...
DO $$ BEGIN
  CREATE POLICY "Authenticated Users Upload Media Files" ON storage.objects FOR INSERT WITH CHECK (
    bucket_id = 'media_uploads' AND auth.role() = 'authenticated'
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
