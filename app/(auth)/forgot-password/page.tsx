...
export default function ForgotPasswordPage() {
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        setErrorMessage("Unable to send password reset email. Please verify your address and try again.");
        setLoading(false);
        return;
      }
    ...
    return (
      <Card className="border-slate-200/80 shadow-md dark:border-slate-800 text-center">
        <CardHeader className="space-y-2">
          ...
          <CardTitle className="text-xl font-bold">Password Reset Email Sent</CardTitle>
          <CardDescription className="text-xs sm:text-sm">
            If an account exists for <strong className="text-slate-900 dark:text-slate-100">{email}</strong>,
  ...
  return (
    <Card className="border-slate-200/80 shadow-md dark:border-slate-800">
      ...
      <CardFooter className="flex justify-center border-t border-slate-100 pt-4 dark:border-slate-800">
        <Link href="/login" className="inline-flex items-center gap-1.5 text-xs text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100 font-medium">
